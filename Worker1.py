    ...
    def recv(self):
        ...
        while True:
            try:
                ...
                try:
                    GZIP_DECOMPRESS = GZIP.decompress(received_data)
                    pickle.loads(GZIP_DECOMPRESS)
        ...
        try:
            ...
            GZIP_DECOMPRESS = GZIP.decompress(received_data)
            GZIP_End = time.time()
            received_data = pickle.loads(GZIP_DECOMPRESS)
            print("  GZIP:   (%.2f)s" %   (GZIP_End - GZIP_Start))
        except BaseException as e:
    ...
    def run(self):
        ...
        for epoch in range(EPOCHS + 1):
            ...
            if subject == "model":
                ...
                train_thread = ThreadWithReturnValue(target=train, args=(model, train_data_loader, test_data_loader, epochs))
                # start the thread
                train_thread.start()
        ...
        resume(model, "IoT_Intrusions_Detection1.pth")
...
# datatest = ReadDataFromCSV('./Datasets/N_BaIoT/5.benign.csv', './Datasets/N_BaIoT/5.gafgyt.combo.csv', './Datasets/N_BaIoT/5.gafgyt.junk.csv', './Datasets/N_BaIoT/5.gafgyt.scan.csv', './Datasets/N_BaIoT/5.gafgyt.udp.csv', './Datasets/N_BaIoT/5.mirai.ack.csv', './Datasets/N_BaIoT/5.mirai.scan.csv', './Datasets/N_BaIoT/5.mirai.syn.csv', './Datasets/N_BaIoT/5.mirai.udp.csv', './Datasets/N_BaIoT/5.mirai.udpplain.csv')
...
def train(model, train_data_loader, test_data_loader, epochs):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if acc > best_accuracy + 1:
            best_accuracy = acc
            best_epoch = epoch
            checkpoint(model, "IoT_Intrusions_Detection1.pth")
